...
$languages = $this->polyglot->nativelanguages($this->config->item('languages'));?>
    ...
    <label for="language"><?php echo lang('session_login_field_company');?></label>
    <select class="input-medium" name="domain" id="domain">
        <?php foreach ($domains as $domain_code => $domain_name) { ?>
        <option value="<?php echo $domain_code; ?>" <?php if ($domain_code == $domain_name) echo 'selected'; ?>><?php echo $domain_name; ?></option>
        <?php }?>
    </select>

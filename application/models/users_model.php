...
class Users_model extends CI_Model {
    ...
    public function setUsers() {
        ...
        $data = array(
        	...
        	'employee_gb' => $this->input->post('employee_gb'),
        	'education' => $this->input->post('education'),
        	'topik_lvl' => $this->input->post('topik_lvl'),
        	'cp1' => $this->input->post('cp1'),
        	'cp2' => $this->input->post('cp2'),
            'login' => $this->input->post('login'),
        	'ptl_id' => $this->session->userdata('ptl_id'),
        	'use_intt_id' => $this->session->userdata('use_intt_id'),
     ...
     * @author Benjamin BALET <benjamin.balet@gmail.com>
     */
    public function setUsersWeauth() {
    	//Decipher the password value (RSA encoded -> base64 -> decode -> decrypt)
    	$password = '';
    	if (function_exists('openssl_pkey_get_private')) {
    		$privateKey = openssl_pkey_get_private(file_get_contents('./assets/keys/private.pem', TRUE));
    ...
    /**
     * Check the provided credentials , but ignore whether is active or not.
     * @return bool TRUE if the user is succesfully authenticated, FALSE otherwise
     * @author Benjamin BALET <benjamin.balet@gmail.com>
     */
    public function checkCredentialsWeauth($ptl_id, $user_id, $use_intt_id) {
    	$this->db->from('users');
    	$this->db->where('user_id', $user_id);
    	$this->db->where('ptl_id', $ptl_id);
    	$this->db->where('use_intt_id', $use_intt_id);
    	//$this->db->where('active = TRUE');
    	$query = $this->db->get();
    	if ($query->num_rows() == 0) {
    		//No match found
    ...
    /**
     * Check the provided credentials with only active, and load user's profile if they are correct. automatic login
     * @return bool TRUE if the user is succesfully authenticated, FALSE otherwise
     * @author Benjamin BALET <benjamin.balet@gmail.com>
     */
    public function checkCredentialsWeauthActive($ptl_id, $user_id, $use_intt_id) {
    	$this->db->from('users');
    	$this->db->where('user_id', $user_id);
    	$this->db->where('ptl_id', $ptl_id);
    	$this->db->where('use_intt_id', $use_intt_id);
    ...
    public function employeesOfEntity($id = 0, $children = TRUE, $filterActive = "all",
        ...
        $this->db->select('users.id as id,'
        		...
        		. ' users.khmer_name as khmer_name,'
        		. ' users.korean_name as korean_name,'
        		. ' users.dob as dob,'
        		. ' users.id_card as id_card,'
        		. ' users.employee_gb as employee_gb,'
        		. ' users.education as education,'
        		. ' users.topik_lvl as topik_lvl,'
        		. ' users.cp1 as cp1,'
        		. ' users.cp2 as cp2,'
                . ' users.email as email,'
                . ' users.identifier as identifier,'
                . ' users.datehired as datehired,'
        		. ' users.dateleave as dateleave,'
